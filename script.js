...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleAuthError(result) {
    if (result && (result.code === 'AUTH_REQUIRED' || result.message === 'Unauthorized' || /unauthorized/i.test(result.message || ''))) {
  ...
  async function loadData() {
    try {
      ...
      if (data.result === 'success') {
        ...
        showToast('Error: ' + (data.message || 'Failed to load'), '#b91c1c');
      }
    } catch (error) {
      console.error('Load error:', error);
      showToast('Failed to load data. Check console.', '#b91c1c');
    } finally {
      showLoading(false);
    }
  }
  ...
  window.deleteApplicant = async function (index) {
    ...
    try {
      ...
      if (result.result === 'success') {
        ...
        if (handleAuthError(result)) return;
        showToast('Error deleting: ' + (result.message || 'Delete failed'), '#b91c1c');
      }
    } catch (error) {
